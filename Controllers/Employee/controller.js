...
async function getAllEmployee(req,resp)
{
    ...
    resp.status(201).send(allEmployee.slice(startIndex,endIndex));
    return;
}
...
async function updateEmployee(req,resp)
{
    ...
    const [isUpdate,msz] =await Employee.update(employetoUpdate,propertyToUpdate,value);
    if(!isUpdate){
        resp.status(403).send(msz)
        return;
    }
...
async function deleteEmployee (req,resp)
{
    ...
    let [findEmployee, isemployeeIdExists] =await  Employee.findEmployeeId(employeeId);
    if(!isemployeeIdExists)
    {
        resp.status(403).send("User not Found");
        return;
    }
    (findEmployee.isActive== true)? (findEmployee.isActive = false) : (findEmployee.isActive = true);
